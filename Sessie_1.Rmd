...
Als je in R een test uitvoert, dan is het resulaat van het type `logical`. We zagen al de functies `is.double` en `is.integer`.
...
Data in R kan in verschillende vormen verschijnen, de zogenaamde data structures. We zullen hieronder een aantal basis structuren behandelen.
...
Net als vectoren kan je elementen in een matrix benaderen met subscripts
...
In R zijn tibles een soort moderne versie van data.frames, afkomstig van Hadley Wickham en er zijn een aantal voordelen om tibbles te gebruiiken. Ze hebben een iets betere print methode en transformeren nooit character naar factor
...
In R zijn lijsten de meest generieke data structuur, het kan beschouwd woorden als een verzanmeling van andere data structuren. Een lijst kan een lijst bevatten, en lijsten zijn soms het resultaat van een functie die allerlei dingen terug geeft in een lijst.
...
Gebruik `function` om zelf een functie te schrijven
...
## ofl als je de functie eerder wilt verlaten met een waarde, gebruik return
f2 = function(x){
  if (x < 0){
    return(1)
  }
...
R heeft ook standaard functies voor het inzelzen van text files, maar ik gebruik vaak `readr`, voor grote files is het veel sneller. En je ziet dan ook een progress bar. In RStudio, kan je de import functies ook aanroepen via de ** Import Dataset ** GUI die het inlezen van data begeleidt.
...
Je kan R objecten (niet alleen data frames) weg schrijven met saveRDS, en deze kunnen weer ingelezen worden met readRDS
