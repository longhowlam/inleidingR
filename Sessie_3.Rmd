...
Deze plots zijn sinds het begin van R al aanwezig, ik noem ze hier alleen heel even, maar ik wil er eigenlijk niet veel aandacht meer aan besteden omdat ik vond dat ze achterhaald zijn. Toch gebruik ik ze stiekem soms wel heel snel.
...
Bar plots zijn handig om bijvoorbeeld aantallen per categorie te visualiseren. Laten we naar top 10 keukens kijken in de iens data set.
...
```{r}
test = restaurants %>% 
  filter(plaats %in% c("Rotterdam", "Utrecht", "Amsterdam")) %>%
  group_by(keuken, plaats) %>%
  summarise(n=n()) %>%
  filter(
    !is.na(keuken)
    ) 
...
Plotly is een zeer mooie library voor het maken van interactive visualisaties. Zie voor meer info [plotly](https://plot.ly/r/). We zullen een aantal voorbeelden hieronder laten zien.
