...
De functie top_n, selecteer de top 4 rijen uit mtcars (op de variable hp).
...
dplyr ondersteunt zogenaamde SQL 'window functions'. Bijvoorbeeld bepaal per cyl value the ranking van de variabele hp.
...
En nu kunnen we ook met de filter functie per cyl waarde de auto nemen met de hoogste hp waarde.
...
Soms wil je twee tabellen op elkaar stappelen tot een nieuwe tabel, dat kan je doen met `bind_rows`. Als je twee tabellen naast elkaar wilt zetten tot een nieuwe tabel, gebruik dan `bind_cols`.
