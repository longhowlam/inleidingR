...
Met `slice` kan je op basis van row numbers data selecteren
...
df2 = data.frame(col1 = c(3,4,5,6,7), xx = rnorm(5), zz = runif(5))

## selecteer rijen die zowel in df1 en df2 zitten 
df3 = inner_join(df1, df2, by = c("col1" = "col1"))

## alle rijen die in df1 zitten, geen match levert NA op
df4 = left_join(df1, df2, by = c("col1" = "col1"))

## alle rijen die in df2 zitten
right_join(df1, df2, by = c("col1" = "col1"))

## alleen de rijen van df1 die niet in df 2 zitten
anti_join(df1, df2, by = c("col1" = "col1"))
...
Een handige functie in tidyr is `separate`, dit kan je gebruiken om een kolom uit elkaar te trekken.
...
Voor datums (en tijden) in R zijn de packages `anydate` en `lubridate` heel handig. Laten we er van uitgaan dat we characters hebben met datums en daar mee willen rekenen. Dan kunnen we de functie `anydate` uit het anytime package gebruiken.
