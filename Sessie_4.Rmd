...
Lineare regressie met de functie `lm`, logistische regressie met de functie `glm`, decision trees met de functie `rpart` en ensemble van trees met `ranger` en  `xgboost`. Ik zal deze functies apart behandelen maar we zullen later in de sessie zien met het package `mlr` hoe je op een meer unforme manier meerdere modellen kan proberen op een data set.


## lineare regressie 

We beginnen met lineare regressie. Modellen waar de Target variable continu (numeric) is. We nemen als voorbeeld de data in mtcars. waarin we horse power (hp) voorspellen op basis van een aantal input variabelen.
...
Een logistic regression is een van de simpeleste predictive modellen om mee te beginnen als je classificatie wilt doen. We gaan uit van een binaire Target (Y /N). We gebruiken de `TTrain` data set die we zojuist gemaakt hebben om een model te fitten.
...
Als je een tree of ensemble van trees hebt getrained kan je een idee krijgen welke variablen in het model belangrijk zijn geweest in het trainings process. Laten we voor de tree modellen die we hierboven getraind hebben de variable importance zien.
...
Naast algemene parameters, heeft elk model bepaalde parameters die je kan zetten. Dit hoeft niet, dan worden defualt waarden gekozen.
...
Als je een benchmark hebt gedaan heb je al de getrainede modellen in het benchmark object zitten. Die kan je al gebruiken om een data set te scoren. je dient dit model er wel 'eerst uit te halen'. 
...
Met de functie `apriori` uit het arules package kan je nu vaak vooromende combinaties van prioducten identificeren.
