...
xgboost.out = xgboost::xgboost(Titan_Inputmatrix, label = TTrain$Survived, nrounds = 10)
...
xgboost.out = xgboost::xgboost(params=param, Titan_Inputmatrix, label = TTrain$Survived, nrounds = 10)
```
...
Als we geen model hebben kan je de overall survival kans uitrekenen:

```{r, eval=FALSE}
## if you know nothing :-)
TTest %>%  summarise(target = mean(Survived))
```

Als we een goed model hebben zullen de survival kansen hoger liggen voor mensen met een hogere score. Latern we de GLM (lons logistische regressie model) in tien stukken (decielen) opdelen, en per deciel de survival kans uitrekenen.
...
## if you have a score!!!
TTest %>% group_by(percPredictie) %>%  summarise(target = mean(Survived))
```
...
## HITRATES 
TTest %>% 
  ggplot(aes(predictieGLM, Survived))  +
  geom_smooth() +
